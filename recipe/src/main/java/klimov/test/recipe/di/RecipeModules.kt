...
import klimov.test.core.navigation.RECIPE_FAVOURITE_SCREEN_QUALIFIER
...
import klimov.test.recipe.vm.RecipeFavouriteViewModel
...
val recipeModules = module {
    ...
    fragment<Fragment>(qualifier = RECIPE_FAVOURITE_SCREEN_QUALIFIER) { RecipeFragment() }

    single { RecipeCoordinator(get<Cicerone<Router>>().router) }
    single { RecipeRepository() }
    factory { RecipeFavouriteViewModel(recipeRepository = get()) }
